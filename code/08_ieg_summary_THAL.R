################################################################################
##  08_ieg_summary_THAL.R                                                      ##
##                                                                             ##
##  Input:  THAL/DEGs_SingleCell_THAL.RData  (da 07_deseq2_THAL)             ##
##  Output: THAL/*.png  (barplot, heatmap IEG, dotplot)                       ##
##                                                                             ##
##  Per ogni timepoint talamica:                                               ##
##    - Conta DEG up/down per tipo cellulare                                  ##
##    - Identifica IEG upregolati per tipo cellulare                          ##
##    - Heatmap LFC IEG × CT (asterischi FDR < 0.05)                         ##
##    - Dotplot: n IEG up + LFC max + FDR min                                ##
##                                                                             ##
##  IEG aggiuntivi rilevanti per il talamo:                                   ##
##    - Npas4: espresso nei TC relay dopo stimolazione                        ##
##    - Btg2, Gadd45b: risposta genica precoce                                ##
##    - Srf: fattore di risposta al siero, attivato da LSD in aree subcortic. ##
################################################################################
...
# Helper per salvare nella subdirectory THAL
...
# IEG standard + addizionali rilevanti per il talamo
IEG_GENES <- c(
  # AP-1 family
  ...
  # Trophic / signaling
  "Npas4","Bdnf", "Ntrk2",
  # DUSPs (inibitori MAPK, attivati precocemente)
  "Dusp1","Dusp5","Dusp6",
  # Altro
  "Rheb", "Homer1","Narp",
  # Aggiuntivi rilevanti per cellule talamiche e sub-corticali
  "Btg2",     # antiproliferativo IEG
  "Gadd45b",  # stress genotossico / plasticità
...
analyze_ieg <- function(degs_list, label) {
  ...
  savepng_thal(p_bar,
               ...
               width = 2500,
               height = max(1800, nrow(deg_counts) * 80 + 600))
  ...
  if (!is.null(ieg_detail) && nrow(ieg_detail) > 0) {
    ...
    for (i in seq_len(nrow(ieg_mat))) {
      ...
      for (j in seq_len(ncol(ieg_mat))) {
        ...
        row_ij <- ieg_detail[ieg_detail$cell_type == ct_i & ieg_detail$gene == g, ]
        if (nrow(row_ij) > 0 && !is.na(row_ij$sig_up[1]) && row_ij$sig_up[1])
          sig_mask[i, j] <- "*"
      }
    }
    ...
    graphics.off()
    png(file.path(thal_dir, paste0("08_IEG_heatmap_", label, ".png")),
    ...
    if (nrow(ieg_pos) > 0) {
      ...
      savepng_thal(p_dot,
                   ...
                   width  = 2800,
                   height = max(1800, nrow(ieg_pos) * 100 + 600))
...
ieg_THAL_90 <- analyze_ieg(DEGs_SC_THAL_90, "THAL_90min")
ieg_THAL_24 <- analyze_ieg(DEGs_SC_THAL_24, "THAL_24h")

message(sprintf("\n=== 08 THAL completato — output in: %s ===", thal_dir))
